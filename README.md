...
|----------|-------------|--------------|
| **Hugo Builder** | Builds your Hugo site and deploys it to GitHub Pages.<br>*Customization*: Configure Git info, Hugo version, and other build parameters. | • `push` to main ✅<br>• `repository_dispatch` ✅<br>• `workflow_dispatch` (manual) ✅ |
| **Hugo Updater** | 1. Checks for new Hugo versions by comparing your `.hugoversion` file with the latest release<br>2. Creates a pull request to update your Hugo version if an update is available<br>3. Automatically merges the PR after creation<br>*Note*: This workflow does NOT run on normal commits - only on schedule or manual triggers to avoid creating too many PRs. | • `schedule` (weekly) ✅<br>• `workflow_dispatch` (manual) ✅<br>• Normal commits/pushes ❌ |
| **PR Merger** | Automatically merges dependency update PRs that meet certain criteria.<br>*Customization*: Configure merge method and PR filtering. | • `pull_request` ✅<br>• `workflow_dispatch` (manual) ✅ |
